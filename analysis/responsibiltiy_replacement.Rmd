...
## Make scatter plot
```{r}
make_scatter = function(data_means, model_name, xlabel, limits) {
...
```{r}
data.exp1 = data.exp1.agent %>%
  mutate(condition = 'agent') %>%
  bind_rows(data.exp1.object %>%
              mutate(condition = 'object'))
```

### Bar plot

```{r fig.width = 8, fig.height = 3}
num_labels = c('0 replacements', '1 replacement', '2 replacements', '3 replacements')
names(num_labels) = c(0, 1, 2, 3)

ggplot(data = data.exp1,
  ...
  geom_point(position = position_jitter(width = 0.1, height = 0),
             ...
             size = 0.5,
             show.legend = F) +
  stat_summary(mapping = aes(y = pred_crm),
               fun = 'mean',
               geom = 'point',
               size = 2,
               shape = 21,
  ...
  facet_wrap(~ num, nrow = 1,
             labeller = labeller(num = num_labels)) +
  scale_fill_manual(values = c('agent' = 'gray90',
                               'objectect' = 'gray75')) +
  theme(text = element_text(size = 12),
...
# ggsave(paste0(figures_dir, 'experiment1/bar.pdf'), width = 8, height = 3)
```
...
labels.exp2 = info.exp2.agent %>% 
  ...
  mutate(condition = NA,
         ...
         grob = map2(.x = low,
                     .y = high,
                     .f = ~ readPNG(paste0(figures_dir, 'experiment2/combos/',
                                           .x, .y, '.png'))))
...
scatter.exp2.agent_uniform = make_scatter(
  data.exp2.agent_means, 'crm_uniform', 'CRM (uniform)', c(40, 100))
scatter.exp2.agent_full = make_scatter(
  data.exp2.agent_means, 'crm_full', 'CRM (full)', c(40, 100))

scatter.exp2.object_uniform = make_scatter(
  data.exp2.object_means, 'crm_uniform', 'CRM (uniform)', c(40, 100))
scatter.exp2.object_full = make_scatter(
  data.exp2.object_means, 'crm_full', 'CRM (full)', c(40, 100))

scatter.exp2.agent_uniform + scatter.exp2.agent_full +
  scatter.exp2.object_uniform + scatter.exp2.object_full +
  plot_layout(ncol = 2) +
...
### Bar plots
```{r}
make_bar.exp2 = function(data, fill, cutoff) {
  g = ggplot(data = data,
             mapping = aes(x = index,
                           y = resp)) +
    stat_summary(fun = "mean",
    ...
    theme(text = element_text(size = 14),
          ...
          panel.grid.major.y = element_line(),
          plot.margin = margin(t = 0.2, l = 0.2, r = 0.2, b = 0.7, unit = "cm")) +
    ...
    coord_cartesian(clip = 'off', ylim = c(cutoff, 100))
...
bar.exp2.agent + bar.exp2.object + 
  plot_layout(ncol = 1) +
  plot_annotation(tag_levels = 'A') &
  theme(plot.tag = element_text(size = 24))

# ggsave(paste0(figures_dir, 'experiment2/bar.pdf'), width = 10, height = 7)
```
...
labels.exp3 = info.exp3.agent %>% 
  ...
  mutate(condition = NA,
         ...
         grob = pmap(.l = list(availability, low, high),
                     .f = ~ readPNG(paste0(figures_dir, 'experiment3/combos/',
                                          ..1, ..2, ..3, '.png'))))
...
scatter.exp3.agent_cp = make_scatter(
  data.exp3.agent_means, 'cp', 'CP model', c(40, 100))
scatter.exp3.agent_crm = make_scatter(
  data.exp3.agent_means, 'crm', 'CRM-based model', c(40, 100))

scatter.exp3.object_cp = make_scatter(
  data.exp3.object_means, 'cp', 'CP model', c(0, 100))
scatter.exp3.object_crm = make_scatter(
  data.exp3.object_means, 'crm', 'CRM-based model', c(0, 100))

scatter.exp3.agent_cp + scatter.exp3.agent_crm +
  scatter.exp3.object_cp + scatter.exp3.object_crm +
  plot_layout(ncol = 2) +
  plot_annotation(tag_levels = list(c('A', '', 'B', ''))) &
  theme(plot.title = element_text(hjust = 0.5),
        plot.tag = element_text(size = 24))
...
### Bar plots
```{r}
make_bar.exp3 = function(data, fill, cutoff) {
  g = ggplot(data = data,
             mapping = aes(x = index,
                           y = resp)) +
    stat_summary(fun = "mean",
    ...
    theme(text = element_text(size = 14),
          ...
          plot.margin = margin(t = 0.2, l = 0.2, r = 0.2, b = 0.8, unit = "cm")) +
    scale_fill_manual(values = c('baseline' = 'black',
                                 'cp' = 'gray50',
                                 'crm' = 'white'),
...
barplot.exp3.agent + barplot.exp3.object +
  plot_layout(ncol = 1) +
  plot_annotation(tag_levels = 'A') &
  theme(plot.tag = element_text(size = 24))
